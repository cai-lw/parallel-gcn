...
AdamVariable::AdamVariable(Variable *var):
    data(&var->data), grad(&var->grad), m(var->size()), v(var->size()) {}

int AdamVariable::size() {
    return data->size();
}

Adam::Adam(std::vector<Variable*> vars, AdamParams params){
    this->params = params;
    for (auto v: vars)
        this->vars.emplace_back(v);
}
...
void Adam::step(){
    ...
    for (auto &var: vars) {
        for (int i = 0; i < var.size(); i++) {
            float grad = (*var.grad)[i] + params.weight_decay * (*var.data)[i];
